...
public final class JavaParserTreeBuilder {
    ...
    private static Optional<Expression> buildMethodCall(TreeNode n) {
        /*
         * Supports both:
         * 1) old local format: MethodInvocation(label="foo", children=[arg0,arg1])
         * 2) Learning-team format: MethodInvocation("", [SCOPE(expr)?, SimpleName(foo), ARGS(args...)])
         */
        if (!n.label().isBlank()) {
            MethodCallExpr callExpr = new MethodCallExpr(n.label());
            NodeList<Expression> args = new NodeList<>();
            for (TreePattern child : n.children()) {
                Optional<Expression> arg = buildExpression(child);
                if (arg.isEmpty()) {
        ...
        Optional<Expression> scope = Optional.empty();
        String methodName = null;
        List<TreePattern> argPatterns = new ArrayList<>();
        ...
        for (TreePattern child : n.children()) {
            ...
            if (childNode.type().equals("SCOPE")) {
                if (childNode.children().size() != 1) {
                    return Optional.empty();
                }
                scope = buildExpression(childNode.children().get(0));
                if (scope.isEmpty()) {
                    return Optional.empty();
                }
                continue;
            ...
            if (isName(childNode.type())) {
                methodName = childNode.label();
                continue;
            }

            if (childNode.type().equals("ARGS") || childNode.type().equals("Arguments") || childNode.type().equals("ArgumentList")) {
                argPatterns.addAll(childNode.children());
                continue;
            }

            // Fallback: if older learned trees store arguments directly.
            argPatterns.add(child);
        }
        ...
        MethodCallExpr callExpr = new MethodCallExpr(methodName);
        scope.ifPresent(callExpr::setScope);

        NodeList<Expression> args = new NodeList<>();
        for (TreePattern argPattern : argPatterns) {
